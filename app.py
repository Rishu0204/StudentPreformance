    ...
    try:
        ...
        try:
            response = client.chat.completions.create(
                model="compound-beta",  # Fast and free model
                messages=[
        ...
        Format your response in a warm, encouraging tone as if speaking directly to the student and their family. Use clear headings and bullet points for easy reading. Keep the response comprehensive but focused (4-5 paragraphs maximum).
            ...
            if getattr(completion, "choices", None):
                try:
                    ...
                    print("AI parse error:", parse_err)
                    ai_result = "AI returned an unexpected response. Please try again."
            else:
                ai_result = "AI returned no choices. Please try again."

        except Exception as groq_err:
            print(f"Groq AI prediction error: {groq_err}")
            ai_result = (
                f"{basic_result}. AI analysis is currently unavailable. "
                "Preliminary suggestions: increase supervised study time, improve nutrition and sleep, "
                "encourage family support, and reduce absences where possible."
            )

    except Exception as e:
        print(f"Error generating AI-powered response: {e}")
        ai_result = "An unexpected error occurred while generating the analysis."
